...
from django.db.models.functions import Cast, TruncDay, TruncMonth, TruncYear
...
from .models import (
    Worker, Attendance, Driver, FuelTransaction, Institution, CarRecords, FountainRecords,
    Maintenance, Expenses, Expenses_type, Place_Expenses, Device,
    MainItem, SubItem, MaintenanceLocation, VehicleFuelRecord, ReportCounter
)
from .forms import (
...
from .exporting import comprehensive_workbook_response, filter_workers, single_report_response
...
def workers_list(request):
    workers = filter_workers(request.GET)
...
def edit_worker_record(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.warning("Worker form errors: %s", form.errors)
    else:
        form = AddWorkerForm(instance=worker_record)

    return render(request, 'workers/edit_worker_record.html', {'form': form, 'worker': worker_record})


@login_required
@manager_only
def delete_worker_image(request, pk):
    worker = get_object_or_404(Worker, pk=pk)
    worker_image = getattr(worker, 'worker_image', None)
    if worker_image:
...
def edit_fuel_record(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.warning("Fuel transaction form errors: %s", form.errors)
    else:
...
def add_car_record(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.warning("Car record form errors: %s", form.errors)
    else:
        form = CarRecordsForm()
...
def add_fountain_record(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.warning("Fountain record form errors: %s", form.errors)
    else:
...
def add_driver(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.warning("Driver form errors: %s", form.errors)
    else:
        form = DriverForm()
...
def edit_maintenance_record(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.warning("Maintenance form errors: %s", form.errors)
    else:
...
def add_expenses(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.warning("Expenses form errors: %s", form.errors)
    else:
        form = ExpensesFrom()
...
def expenses_list(request):
    ...
    if end_date:
        records = records.filter(date__lte=end_date)
    if place_form:
        records = records.filter(palce__id=place_form)
...
def add_vehicle_fuel_record(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.warning("Vehicle fuel form errors: %s", form.errors)
    else:
...
@login_required
@manager_only
def export_workers_excel(request):
    return single_report_response("workers", request.GET)


@login_required
@manager_only
def export_attendance_report_excel(request):
    return single_report_response("attendance", request.GET)


@login_required
@manager_only
def export_workers_delays_excel(request):
    return single_report_response("workers_delays", request.GET)


@login_required
@manager_only
def export_drivers_excel(request):
    return single_report_response("drivers", request.GET)


@login_required
@manager_only
def export_institutions_excel(request):
    return single_report_response("institutions", request.GET)


@login_required
@manager_only
def export_fuel_transactions_excel(request):
    return single_report_response("fuel", request.GET)


@login_required
@manager_only
def export_car_records_excel(request):
    return single_report_response("cars", request.GET)


@login_required
@manager_only
def export_fountain_records_excel(request):
    return single_report_response("fountain", request.GET)


@login_required
@manager_only
def export_maintenance_excel(request):
    return single_report_response("maintenance", request.GET)


@login_required
@manager_only
def export_expenses_excel(request):
    return single_report_response("expenses", request.GET)


@login_required
@manager_only
def export_vehicle_fuel_excel(request):
    return single_report_response("vehicle_fuel", request.GET)


@login_required
@manager_only
def export_financial_report_excel(request):
    return single_report_response("financial", request.GET)


@login_required
@manager_only
def comprehensive_export_page(request):
    return render(request, 'reports/comprehensive_export.html')


@login_required
@manager_only
def download_comprehensive_excel(request):
    return comprehensive_workbook_response()
